...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        temp = np.matmul(prev_h,self.params[self.wh_name]) + np.matmul(x,self.params[self.wx_name])
        meta = temp + self.params[self.b_name]
        next_h = np.tanh(meta)
        meta = [x,next_h,prev_h]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        return next_h, meta
    ...
    def step_backward(self, dnext_h, meta):
        ...
        # corresponding name.                                                       #
        #############################################################################
        x, next_h,prev_h = meta
        dloc = 1 - np.square(next_h)
        dloc = dloc * dnext_h
        db = np.matmul(np.transpose(dloc),np.ones(np.shape(dloc)[0]))
        dprev_h = np.matmul(dloc,np.transpose(self.params[self.wh_name]))
        dWh = np.matmul(np.transpose(prev_h),dloc)
        dWx = np.matmul(np.transpose(x),dloc)
        dx = np.matmul(dloc, np.transpose(self.params[self.wx_name]))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def forward(self, x, h0):
        ...
        temp = h0
        for t in range(T):
            temp , temp1 = self.step_forward(x[:,t,:],temp)
            h[:,t,:] = temp
            self.meta.append(temp1)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return h
    ...
    def backward(self, dh):
        ...
        ##############################################################################
        dx = np.empty((dh.shape[0],dh.shape[1],self.input_dim))
        dprev_ht = 0
        dWx =0
        dWh = 0
        db = 0
        for t in reversed(range(dh.shape[1])):
            temp = dprev_ht + dh[:,t,:]
            dxt, dprev_ht, dWxt, dWht, dbt = self.step_backward(temp,self.meta[t])
            dWx = dWx+dWxt
            dWh = dWh+dWht
            db = db + dbt
            dx[:,t,:] = dxt
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        #############################################################################
        temp = np.matmul(x,self.params[self.wx_name]) + np.matmul(prev_h,self.params[self.wh_name]) + self.params[self.b_name];
        i = sigmoid(temp[:,0:self.h_dim])
        f = sigmoid(temp[:,self.h_dim:2*self.h_dim])
        o = sigmoid(temp[:,2*self.h_dim:3*self.h_dim])
        g = np.tanh(temp[:,3*self.h_dim:4*self.h_dim ])

        next_c = np.multiply(f,prev_c) + np.multiply(i,g)
        next_h = np.multiply(o,np.tanh(next_c))

        meta = [x,prev_c,next_c,prev_h,next_h,i,f,o,g]
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        dx: gradients of input feature (N, D)
        dprev_h: gradients of previous hiddel state (N, H)
        dWh: gradients w.r.t. feature-to-hidden weights (H, 4H)
        dWx: gradients w.r.t. hidden-to-hidden weights (D, 4H)
        db: gradients w.r.t bias (H,)
        """
        ...
        #############################################################################
        x,prev_c,next_c,prev_h,next_h,i,f,o,g = meta
        ...
        net = np.hstack((di,df,do,dg))
        db = np.sum(net,axis=0)

        dprev_h = np.matmul(net,np.transpose(self.params[self.wh_name]))
        dx = np.matmul(net,np.transpose(self.params[self.wx_name]))

        dWx = np.matmul(x.T,net)
        dWh = np.matmul(prev_h.T,net)
    ...
    def forward(self, x, h0):
        ...
        #############################################################################
        prev_c = np.zeros((x.shape[0],self.h_dim))
        prev_h = h0
        h = np.zeros((x.shape[0],x.shape[1],self.h_dim))
        for t in range(x.shape[1]):
            next_h, next_c, temp = self.step_forward(x[:,t,:], prev_h, prev_c)
            h[:,t,:] = next_h
            prev_h = next_h
            prev_c = next_c
    ...
    def backward(self, dh):
        ...
        dWx = 0# np.zeros((self.input_dim, 4*self.h_dim))
        dWh = 0  #np.zeros((self.h_dim, 4*self.h_dim))
        db = 0 #np.zeros((4*self.h_dim))
        dprev_ht =  0#np.zeros((dh.shape[0], dh.shape[2]))
        #print(np.shape(self.meta))
        for t in reversed(range(dh.shape[1])):
            dprev_h = dh[:,t,:] + dprev_ht
            dxt, dprev_ht, dprev_ct, dWxt, dWht, dbt = self.step_backward(dprev_h, dnext_c, self.meta[t])
            dx[:,t,:] = dxt
            dWx += dWxt
            dWh += dWht
            db += dbt
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[0]):
            for j in range(x.shape[1]):
                temp = np.zeros(self.voc_dim)
                temp[x[i,j]]=1
                out[i,j,:] = np.matmul(np.transpose(self.params[self.w_name]),temp)
